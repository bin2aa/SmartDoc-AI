...
class OllamaLLMService(AbstractLLMService):
    ...
    def generate_stream(self, prompt: str, **kwargs) -> Generator[str, None, None]:
        """
        Stream response tokens from prompt.

        Yields tokens as they are generated by the LLM, preventing the
        UI from appearing frozen during long generations.  Includes the
        same model-not-found and memory-error fallback logic as
        :meth:`generate`.
        ...
        Raises:
            LLMConnectionError: If streaming fails after all retries
        """
        try:
            logger.info(f"Streaming response for prompt length: {len(prompt)}")
        ...
        # ── Fallback handling (outside the yield-try above) ───────────
        # We cannot yield inside the nested try/except, so handle retries
        # by delegating to generate() which returns a complete string and
        # then streaming that string token-by-token.
        ...
        if self._is_memory_error(error_message):
            logger.warning(
                "Memory pressure during streaming. Falling back to generate(). "
                f"model={self.model}"
            )
            try:
                # generate() already has memory-fallback logic
                fallback_response = self.generate(prompt, **kwargs)
                ...
                raise LLMConnectionError(
                    f"Failed to stream or generate response (memory error): {fallback_err}"
                )

        raise LLMConnectionError(f"Failed to stream response: {error_message}")
